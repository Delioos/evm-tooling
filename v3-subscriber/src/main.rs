...
async fn main() -> Result<()> {
    ...
    println!("connecting to {}", "altitude".black().on_cyan());
    
    // Subscribe to Swap events of v3 pools
   
    let signature = keccak256("Swap(address, address, int256, int256, uint160, uint128, int24)");
    println!("Subscribing to events with signature {}", signature.to_string().black().on_red());
    ...
    while let Some(log) = stream.next().await {
        println!("Raw log: {:#?}", log);
        // build the primitive type log 
        let primitive_log = alloy::primitives::Log::from(log.inner);

        let decoded = IUniswapV3PoolEvents::Swap::decode_log(&primitive_log, true).unwrap();
